...
for my $i (0..$#{$mod}) {
    ...
    my $meta_file = "$run_dir/meta.yaml";
    ...
    $final =~ /\$DATA\s+(\S+)/;
    my $input_data_file = $1;
    $mod->[$i]{"dataset"} = $input_data_file;
    ...
    my $namemap_yaml;
    open NAMEMAP_YAML, ">", \$namemap_yaml;
    print NAMEMAP_YAML "\n";
    print NAMEMAP_YAML "  theta:\n";
    for my $i (0 .. $#THETA_name_order) {
        print NAMEMAP_YAML "    - $THETA_name_order[$i]\n";
    }
    print NAMEMAP_YAML "  sigma:\n";
    for my $i (0 .. $#EPS_name_order) {
        print NAMEMAP_YAML "    - $EPS_name_order[$i]\n";
    }
    print NAMEMAP_YAML "  omega:\n";
    for my $i (0 .. $#ETA_name_order) {
        print NAMEMAP_YAML "    - $ETA_name_order[$i]\n";
    }
    $mod->[$i]{"NAMEMAP"} = $namemap_yaml;

    my $df_yaml;
    open DF_YAML, ">", \$df_yaml;
    print DF_YAML "\n";
    print DF_YAML "  theta: $THETA_df\n";
    print DF_YAML "  sigma: $SIGMA_df\n";
    print DF_YAML "  omega: $OMEGA_df\n";
    print DF_YAML "  total: @{[$THETA_df + $SIGMA_df + $OMEGA_df]}\n";
    $mod->[$i]{"DF"} = $df_yaml;

    print "Generating $meta_file ...\n";
    $tt->process(\$meta, $mod->[$i], $meta_file) || die $tt->error;
...
echo rmarkdown::render("results.R") | "R.exe" --no-save
...
$template_menu_run_all_seq_bat = <<'END_TEMPLATE_RUN_ALL_SEQ_BAT';
@echo off
[% FOREACH m IN models %]
call nmrun_[% m.name %].bat
[% END %]
END_TEMPLATE_RUN_ALL_SEQ_BAT

$tt->process(\$template_menu_run_all_seq_bat, { models => $mod }, "nmrun_all_sequential.bat") || die $tt->error;
...
$tt->process(\$template_menu_run_all_seq_sh, { models => $mod }, "nmrun_all_sequential.sh") || die $tt->error;
...
rem echo  p) Run all the models in parallel (caution: don't use if there are many models)
echo -
...
rem :executeallpara
rem echo Executing menu_run_all_parallel 
rem call nmrun_all_parallel.bat
rem goto menu
