...
for my $i (0..$#{$mod}) {
    ...
    my $run_dir = "$modname";
    my $ctl_file = "$run_dir/${modname}_vpc.ctl";
    #my $namemap_file = "$run_dir/NAMEMAP";
    #my $df_file = "$run_dir/DEGREES_OF_FREEDOM";
    my $bat_file = "nmrun_${modname}_vpc.bat";
...
\@echo off
title Running model ${modname}_vpc
call nmexec_vpc ${modname}
END_NMRUN_BAT
    ...
    $render_vpc_bat = <<END_RENDER_VPC_BAT;
\@echo off
copy ..\\vpc.R .\\vpc.R
echo rmarkdown::render("vpc.R") | "R.exe" --no-save
for %%a in (.) do set rundir=%%~na
move vpc.html %rundir%_vpc.html
END_RENDER_VPC_BAT
...
END_RUN_ME_SH
    ...
    open RUNMEDOTSH, ">", "$run_dir/RUN_VPC.sh" || die "Error opening file $run_dir/RUN_VPC.sh in write mode: $!";
