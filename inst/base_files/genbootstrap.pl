...
for my $i (0..$#{$mod}) {
    ...
    next if not exists $mod->[$i]{'bootstrap'};

    my $modname = $mod->[$i]{'name'};
    my $run_dir = "$modname";
    my $ctl_file = "$run_dir\\Bootstrap\\${modname}.ctl";
    my $lst_file = "$run_dir\\${modname}.lst";
    #my $namemap_file = "$run_dir/NAMEMAP";
    #my $df_file = "$run_dir/DEGREES_OF_FREEDOM";
    my $bat_file = "$run_dir\\Bootstrap\\RUN_ME.sh";
    ...
    unless(-d "$run_dir\\Bootstrap") {
        print "Creating directory $run_dir\\Bootstrap ...\n";
        mkdir "$run_dir\\Bootstrap" or die "Error creating directory $run_dir\\Bootstrap: $!";
    }

    if (-f "$lst_file") {
        print "Copying $lst_file to $run_dir\\Bootstrap ...\n";
        copy("$lst_file", "$run_dir\\Bootstrap") || die "Copy failed: $!";
    } else {
        print "$lst_file does not exist!\n";
    }
...
rm -rf bootstrap_dir*
bootstrap --run_on_sge \$MODEL.ctl --nm_version=nm73 --threads=$nnodes --samples=\$SAMPLES --seed=2943 --stratify_on="PROT" --clean=4 --rplots=0
...
END_NMRUN_BAT
    ...
    $render_bootstrap_bat = <<END_RENDER_BOOTSTRAP_BAT;
\@echo off
copy ..\\..\\bootstrap.R .\\bootstrap.R
echo rmarkdown::render("bootstrap.R") | "R.exe" --no-save
for %%a in (..) do set rundir=%%~na
move bootstrap.html %rundir%_bootstrap.html
END_RENDER_BOOTSTRAP_BAT

    open RENDERBOOTSTRAPBAT, ">", "$run_dir/Bootstrap/render_bootstrap.bat" || die "Error opening file $run_dir/render_bootstrap.bat in write mode: $!";
    print RENDERBOOTSTRAPBAT $render_bootstrap_bat;

    #if (-f "bootstrap.R") {
    #    copy("bootstrap.R", "$run_dir/bootstrap.R") || die "Copy failed: $!";
    #}
    #if (-f "render_bootstrap.bat") {
    #    copy("render_bootstrap.bat", "$run_dir/render_bootstrap.bat") || die "Copy failed: $!";
    #}
}
